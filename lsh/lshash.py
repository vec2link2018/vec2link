...
class LSHash(object):
    ...
    def index(self, input_point, extra_data=None):
        ...
        for i, table in enumerate(self.hash_tables):
            key = self._hash(self.uniform_planes[i], input_point)
            table.append_val(key, value)
            self.hamming_code_results.append(map(int, list(key)))

    def hamming_code(self):
        return self.hamming_code_results
